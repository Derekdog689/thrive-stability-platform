...
const statusVisuals: Record<GoalProgressStatus, { dot: string; badge: string }> = {
  not_started: { dot: "bg-slate-400", badge: "bg-slate-100 text-slate-700" },
  in_progress: { dot: "bg-emerald-500", badge: "bg-emerald-50 text-emerald-800" },
  paused: { dot: "bg-amber-400", badge: "bg-amber-50 text-amber-900" },
  completed: { dot: "bg-sky-500", badge: "bg-sky-50 text-sky-800" },
  archived: { dot: "bg-slate-300", badge: "bg-slate-100 text-slate-500" },
};
  ...
  return <article className="overflow-hidden rounded-[2rem] border border-white/80 bg-white/70 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-2xl">
    <div className="relative p-6 sm:p-8">
      ...
      <div className="relative">
        ...
        <div className="mt-6 flex items-center gap-3"><span className={`h-4 w-4 rounded-full ${visual.dot}`} /><p className="text-sm font-black uppercase tracking-wide text-slate-500">Where it stands</p></div>
        <div className="mt-5 rounded-[1.6rem] border border-emerald-100 bg-emerald-50/80 p-5"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700">Next</p><p className="mt-2 text-2xl font-black leading-8 text-emerald-950">{goal.next_step}</p></div>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          {goal.progress_status === "not_started" ? <button type="button" disabled={working} onClick={() => void onStatusChange(goal, "in_progress")} className="rounded-full bg-emerald-700 px-6 py-3 font-black text-white shadow-sm disabled:opacity-60">Start</button> : null}
          {goal.progress_status === "in_progress" ? <><button type="button" disabled={working} onClick={() => void onStatusChange(goal, "completed")} className="rounded-full bg-emerald-700 px-6 py-3 font-black text-white disabled:opacity-60">Done</button><button type="button" disabled={working} onClick={() => void onStatusChange(goal, "paused")} className="rounded-full border border-amber-200 bg-amber-50 px-5 py-3 font-black text-amber-900 disabled:opacity-60">Pause</button></> : null}
          {goal.progress_status === "paused" ? <button type="button" disabled={working} onClick={() => void onStatusChange(goal, "in_progress")} className="rounded-full bg-emerald-700 px-6 py-3 font-black text-white disabled:opacity-60">Continue</button> : null}
        </div>
      </div>
    </div>
  </article>;
...
export default function GoalsCandidatePage() {
  ...
  return <AuthGate><main className="min-h-screen bg-[radial-gradient(circle_at_12%_12%,rgba(167,243,208,0.32),transparent_30%),radial-gradient(circle_at_86%_18%,rgba(254,240,138,0.28),transparent_26%),linear-gradient(180deg,#edf7f1_0%,#eef5f7_48%,#edf1f4_100%)] px-3 pb-28 pt-3 text-slate-950 sm:px-6 sm:pb-32 sm:pt-6"><section className="mx-auto max-w-5xl space-y-5 sm:space-y-6">
    ...
    {!loading && !errorMessage && canCreate ? <>
      {justSavedGoal ? <section className="rounded-[2rem] border border-white/80 bg-white/72 p-6 shadow-sm backdrop-blur-2xl sm:p-8"><p className="text-[11px] font-black uppercase tracking-[0.2em] text-emerald-700">Saved</p><h2 className="mt-2 text-3xl font-black">{justSavedGoal.title}</h2><div className="mt-5 rounded-[1.6rem] bg-emerald-50 p-5"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700">Start here</p><p className="mt-2 text-2xl font-black">{justSavedGoal.next_step}</p></div><div className="mt-5 flex flex-wrap gap-3">{justSavedGoal.progress_status === "not_started" ? <button type="button" disabled={working} onClick={() => void changeStatus(justSavedGoal, "in_progress")} className="rounded-full bg-emerald-700 px-6 py-3 font-black text-white">Start</button> : null}<button type="button" onClick={() => setJustSavedGoal(null)} className="rounded-full border border-slate-200 bg-white px-5 py-3 font-black">My goals</button><Link href="/" className="rounded-full border border-slate-200 bg-white px-5 py-3 font-black">Done for now</Link></div></section> : null}

      {!justSavedGoal && currentGoals.length > 0 ? <section className="space-y-4"><div className="px-1"><p className="text-[11px] font-black uppercase tracking-[0.2em] text-emerald-700">Right now</p><h2 className="mt-2 text-3xl font-black">Your goals</h2></div>{currentGoals.map((goal) => <ActiveGoalCard key={goal.id} goal={goal} working={working} onStatusChange={changeStatus} />)}</section> : null}
      ...
      {!justSavedGoal && showCreate ? <form onSubmit={saveGoal} className="rounded-[2rem] border border-white/80 bg-white/78 p-5 shadow-sm backdrop-blur-2xl sm:p-8"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700">Goal</p><p className="mt-1 text-sm font-bold text-slate-500">{step} of 4</p></div><button type="button" onClick={resetCreation} className="rounded-full border border-slate-200 px-4 py-2 text-sm font-black">Close</button></div>
        {step === 1 ? <div className="mt-7"><h2 className="text-3xl font-black">Pick an area.</h2><div className="mt-5 grid grid-cols-2 gap-3">{goalAreas.map((area) => { const visual = goalAreaVisuals[area.id] ?? { symbol: "•", label: area.label }; return <button key={area.id} type="button" onClick={() => chooseArea(area.id)} className="flex min-h-32 flex-col items-center justify-center rounded-[1.75rem] border border-slate-200 bg-white/80 p-4 text-center transition hover:border-emerald-300 hover:bg-emerald-50"><span className="text-3xl font-black">{visual.symbol}</span><span className="mt-3 font-black">{visual.label}</span></button>; })}</div></div> : null}
        {step === 2 && selectedArea ? <div className="mt-7"><button type="button" onClick={() => setStep(1)} className="text-sm font-black text-slate-500">← Back</button><h2 className="mt-4 text-3xl font-black">Pick a goal.</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{selectedArea.presets.map((preset) => <button key={preset.id} type="button" onClick={() => choosePreset(preset.id)} className="rounded-2xl border border-slate-200 bg-white p-5 text-left text-lg font-black hover:border-emerald-300 hover:bg-emerald-50">{preset.title}</button>)}</div><button type="button" onClick={() => { setDraft((current) => ({ ...current, presetId: "", title: "" })); setStep(3); }} className="mt-4 rounded-full border border-emerald-200 bg-emerald-50 px-5 py-3 font-black text-emerald-900">Write my own</button></div> : null}
        {step === 3 ? <div className="mt-7"><button type="button" onClick={() => setStep(2)} className="text-sm font-black text-slate-500">← Back</button>{!draft.presetId ? <label className="mt-4 block text-sm font-black">Goal<input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} maxLength={180} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 font-normal outline-none focus:border-emerald-500" placeholder="What are you working toward?" /></label> : <div className="mt-4 rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Goal</p><p className="mt-2 text-xl font-black">{draft.title}</p></div>}<h2 className="mt-6 text-3xl font-black">Pick one first step.</h2>{selectedPreset ? <div className="mt-5 grid gap-3 sm:grid-cols-2">{selectedPreset.nextSteps.filter((value) => value !== "Write my own next step").map((value) => <button key={value} type="button" onClick={() => chooseNextStep(value)} className="rounded-2xl border border-slate-200 bg-white p-5 text-left font-black hover:border-emerald-300 hover:bg-emerald-50">{value}</button>)}</div> : null}<label className="mt-5 block text-sm font-black">Or write one<input value={draft.nextStep} onChange={(event) => setDraft((current) => ({ ...current, nextStep: event.target.value }))} maxLength={240} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 font-normal outline-none focus:border-emerald-500" placeholder="One useful next step" /></label><button type="button" disabled={!draft.title.trim() || !draft.nextStep.trim()} onClick={() => setStep(4)} className="mt-5 rounded-full bg-emerald-700 px-6 py-3 font-black text-white disabled:opacity-40">Review</button></div> : null}
        {step === 4 ? <div className="mt-7"><button type="button" onClick={() => setStep(3)} className="text-sm font-black text-slate-500">← Back</button><h2 className="mt-4 text-3xl font-black">Save your goal.</h2><div className="mt-5 rounded-[1.6rem] bg-slate-50 p-5"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Goal</p><p className="mt-2 text-xl font-black">{draft.title}</p></div><div className="mt-3 rounded-[1.6rem] bg-emerald-50 p-5"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700">First step</p><p className="mt-2 text-xl font-black">{draft.nextStep}</p></div><label className="mt-5 block text-sm font-black">Why it matters <span className="font-normal text-slate-400">Optional</span><textarea value={draft.why} onChange={(event) => setDraft((current) => ({ ...current, why: event.target.value }))} maxLength={500} rows={3} className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 font-normal outline-none focus:border-emerald-500" placeholder="Anything worth remembering" /></label><button type="submit" disabled={working || !draft.title.trim() || !draft.nextStep.trim()} className="mt-5 rounded-full bg-emerald-700 px-6 py-3 font-black text-white disabled:opacity-40">{working ? "Saving..." : "Save goal"}</button></div> : null}
