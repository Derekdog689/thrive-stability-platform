...
function daySignal(value: string | null | undefined) {
  switch (value) {
    case "good":
      return { label: "Feeling good", level: 4 };
    case "okay":
      return { label: "Doing alright", level: 3 };
    case "hard":
      return { label: "Hard day", level: 1 };
    case "not_sure":
      return { label: "Not sure today", level: 2 };
    default:
      return { label: "Check in", level: 0 };
  }
}
...
export default function TodayPage() {
  ...
  [...recentCheckins]
    ...
    .forEach((checkin) => checkinByDate.set(checkin.checkin_date, checkin));
  ...
  const weekDays = Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      ...
      checkin,
      signal: daySignal(checkin?.overall_day),
    };
  });
          ...
          <header className="thrive-ambient relative overflow-hidden rounded-[2rem] border border-white/65 bg-white/28 px-5 py-4 shadow-[0_18px_50px_rgba(15,23,42,0.09)] backdrop-blur-2xl sm:px-7 sm:py-5">
            ...
            <div className="relative z-10 flex items-center justify-between gap-3">
              ...
              <button
                ...
                disabled={signingOut}
                className="rounded-full border border-white/80 bg-white/55 px-3 py-2 text-[11px] font-black text-emerald-950 backdrop-blur-xl transition active:scale-95 disabled:opacity-60"
              >
            ...
            <div className="relative z-10 mt-5 flex items-end justify-between gap-4">
              <div className="min-w-0">
                <p className="font-serif text-lg text-emerald-950 sm:text-2xl">{timeGreeting},</p>
                <h1 className="truncate font-serif text-4xl font-black tracking-tight text-emerald-950 sm:text-6xl">
                  {participantName || "there"}
                </h1>
              </div>
              <Link
                href="/wellness"
                className="flex shrink-0 items-center gap-2 rounded-full border border-white/75 bg-white/58 px-3 py-2 text-xs font-black text-slate-800 backdrop-blur-xl transition active:scale-95"
              >
                <Icon name="wellness" className="h-5 w-5" />
                <span className="max-w-24 truncate">{isNewParticipant ? "Check in" : signal.label}</span>
              </Link>
            </div>
          </header>

          {errorMessage ? (
            <section role="alert" className="mt-3 rounded-3xl border border-rose-200 bg-rose-50/90 p-4 text-rose-950 shadow-sm">
          ...
          {!isNewParticipant ? (
            <section className="mt-3 grid grid-cols-4 gap-2">
              <Link href="/wellness" className="thrive-glance thrive-glance-compact border-emerald-100/70 bg-emerald-50/58 text-emerald-950">
                <Icon name="wellness" className="h-6 w-6" />
                <span>Wellness</span>
                <strong>{todayCheckin ? formatChoice(todayCheckin.overall_day) : "Check in"}</strong>
              </Link>
              <Link href="/goals" className="thrive-glance thrive-glance-compact border-amber-100/70 bg-amber-50/58 text-amber-950">
                <Icon name="goal" className="h-6 w-6" />
                <span>Goal</span>
              ...
              <Link href="/support" className="thrive-glance thrive-glance-compact border-violet-100/70 bg-violet-50/58 text-violet-950">
                ...
                <span>Support</span>
                <strong>{supportNeedsParticipant ? "Reply" : unresolvedSupportRequest ? "Open" : "Ready"}</strong>
              </Link>
            </section>
          ) : null}
          ...
          <section className="mt-3 grid gap-3 lg:grid-cols-[1.05fr_.95fr]">
            <article className="thrive-focus-card relative overflow-hidden rounded-[1.8rem] border border-white/70 bg-white/46 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.09)] backdrop-blur-2xl sm:p-6">
              ...
              <div className="relative z-10 flex items-start gap-3">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/80 bg-white/72 text-emerald-900 shadow-sm">
                  <Icon name={primaryAction.icon} className="h-6 w-6" />
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-2 text-base font-semibold leading-6 text-slate-600">{primaryAction.detail}</p>
                </div>
              </div>
              <Link
                href={primaryAction.href}
                className="relative z-10 mt-4 flex min-h-12 w-full items-center justify-center rounded-[1.2rem] bg-emerald-700 px-4 py-3 text-center text-base font-black text-white shadow-[0_10px_24px_rgba(4,120,87,0.22)] transition hover:bg-emerald-800 active:scale-[0.985]"
              >
                {primaryAction.action}<Icon name="arrow" className="ml-2 h-5 w-5" />
              </Link>
            </article>
            ...
            <article className="rounded-[1.8rem] border border-white/70 bg-white/46 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-2xl sm:p-6">
              ...
              {activeBudgetPeriod ? (
                <div className="mt-5">
                  <div className="relative h-3 overflow-hidden rounded-full bg-white/80 shadow-inner">
                    <div
                  ...
                  <div className="mt-3 flex items-end justify-between gap-4">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-wide text-slate-500">Out</p>
                      <p className="mt-1 text-xl font-black">{formatMoneyShort(moneyOutThisPeriod)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] font-black uppercase tracking-wide text-slate-500">Plan</p>
                      <p className="mt-1 text-xl font-black">{formatMoneyShort(moneyPlanTotal)}</p>
                    </div>
                  </div>
                ...
                <Link href="/budget" className="mt-5 inline-flex rounded-full bg-cyan-50 px-4 py-2 font-black text-cyan-900">Set up money plan</Link>
              )}
            </article>
          </section>
            ...
            <section className="mt-3 grid gap-3 lg:grid-cols-[1fr_.62fr]">
              <article className="rounded-[1.8rem] border border-white/70 bg-white/42 p-4 shadow-[0_16px_45px_rgba(15,23,42,0.07)] backdrop-blur-2xl sm:p-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700">Your week</p>
                    <h2 className="mt-1 text-xl font-black">Check-ins</h2>
                  </div>
                  <Link href="/wellness" className="text-sm font-black text-emerald-800">Wellness ›</Link>
                </div>

                <div className="mt-5 grid grid-cols-7 gap-1.5">
                  {weekDays.map((day) => (
                    <div key={day.dateKey} className="flex min-w-0 flex-col items-center gap-2">
                      <div className="flex h-14 w-full items-end justify-center rounded-[1rem] bg-white/55 px-1 pb-1.5 shadow-sm">
                        <div
                          className={`w-full max-w-5 rounded-full transition-all duration-500 ${day.checkin ? "bg-emerald-600" : "bg-slate-200/70"}`}
                          style={{ height: day.checkin ? `${24 + day.signal.level * 6}%` : "14%" }}
                          title={day.checkin ? day.signal.label : "No check-in"}
                        />
                      </div>
                      <span className="text-[9px] font-black uppercase text-slate-500">{day.day}</span>
                    </div>
                  ))}
                </div>
                <p className="mt-4 text-sm font-bold text-slate-600">
                  {weekCheckinCount === 0 ? "No check-ins this week" : `${weekCheckinCount} check-in${weekCheckinCount === 1 ? "" : "s"} this week`}
                </p>
              </article>

              {supportNeedsParticipant ? (
                <article className="rounded-[1.8rem] border border-violet-200 bg-violet-50/68 p-4 shadow-[0_16px_45px_rgba(15,23,42,0.07)] backdrop-blur-2xl sm:p-6">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/75 text-violet-800 shadow-sm">
                      <Icon name="support" className="h-6 w-6" />
                    </div>
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.18em] text-violet-700">Support</p>
                      <h2 className="mt-1 text-lg font-black">Reply waiting</h2>
                    </div>
                  </div>
                  <Link href="/support" className="mt-4 flex min-h-11 items-center justify-center rounded-[1.1rem] bg-white/78 px-4 py-2.5 font-black text-violet-900 transition active:scale-[0.985]">
                    See reply
                  </Link>
                </article>
              ) : (
                <Link href="/support" className="flex min-h-20 items-center justify-between rounded-[1.8rem] border border-white/70 bg-white/38 px-4 py-3 text-slate-800 shadow-[0_14px_38px_rgba(15,23,42,0.06)] backdrop-blur-2xl transition active:scale-[0.99] sm:px-6">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/72 text-violet-800 shadow-sm">
                      <Icon name="support" className="h-6 w-6" />
                    </div>
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.18em] text-violet-700">Support</p>
                      <p className="mt-0.5 font-black">{unresolvedSupportRequest ? "Request open" : "Available"}</p>
                    </div>
                  </div>
