...
-- Installation version: v0.2
-- Scope: FUNCTION DEFINITION AND RESTRICTED EXECUTION GRANT ONLY
--
-- This installation does not:
-- - call the function;
-- - import January data;
-- - create a financial source;
-- - create an import batch;
-- - create staged transactions;
-- - approve or review financial records.
--
-- The application environment gate must remain false.
...
--
-- v0.2 corrections:
-- - serialize imports for the locked workspace, program, and file hash
-- - reject ambiguous matching financial sources
-- - require the exact source-row range 2 through 86
-- - preserve atomic rollback on every validation failure
