...
-- Installation candidate version: v0.1
-- Status: REVIEW-ONLY PERMANENT FIXTURE INSTALLATION CANDIDATE
-- DO NOT EXECUTE WITHOUT SEPARATE PERMANENT INSTALLATION APPROVAL
--
-- Purpose:
...
-- - install anything unless this exact artifact receives separate approval.
--
...
-- ============================================================
-- 3. Mandatory installation conflict guard
-- ============================================================
...
begin
  ...
  if conflict_count <> 0 then
    raise exception
      'INSTALLATION STOP: expected zero conflicting controlled fixture records, found %',
      conflict_count;
  end if;
end;
$function$;
...
-- ============================================================
-- 7. Supported-person test records
-- ============================================================
...
-- ============================================================
-- 8. Program-participation test records
-- ============================================================
...
-- ============================================================
-- 10. Transactional installation assertions
-- ============================================================
...
begin
  ...
  if workspace_count <> 1 then
    raise exception
      'INSTALLATION FAILURE: expected 1 test workspace, found %',
      workspace_count;
  end if;

  if membership_count <> 5 then
    raise exception
      'INSTALLATION FAILURE: expected 5 test memberships, found %',
      membership_count;
  end if;

  if outsider_membership_count <> 0 then
    raise exception
      'INSTALLATION FAILURE: outsider unexpectedly received a membership';
  end if;

  if program_count <> 1 then
    raise exception
      'INSTALLATION FAILURE: expected 1 test program, found %',
      program_count;
  end if;

  if supported_person_count <> 2 then
    raise exception
      'INSTALLATION FAILURE: expected 2 supported people, found %',
      supported_person_count;
  end if;

  if participant_count <> 2 then
    raise exception
      'INSTALLATION FAILURE: expected 2 program participants, found %',
      participant_count;
  end if;

  if profile_count <> 0 then
    raise exception
      'INSTALLATION FAILURE: expected 0 synthetic test profile rows, found %',
      profile_count;
  end if;
end;
$function$;
...
-- ============================================================
-- 11. Permanent installation boundary
-- ============================================================
--
-- The transaction commits only if every guard, insert, constraint,
-- trigger, and assertion above succeeds.

commit;

-- ============================================================
-- 12. Immediate post-install verification
-- ============================================================
