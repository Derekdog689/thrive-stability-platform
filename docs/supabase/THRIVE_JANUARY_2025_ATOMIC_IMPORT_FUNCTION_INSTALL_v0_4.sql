...
-- Installation version: v0.4
-- Corrects ambiguous import_status reference.
-- This script installs the function but does not call it.
...
--
-- v0.4 corrections:
-- - preserve all prior transaction and validation controls
-- - qualify the batch table in the final status transition
-- - remove ambiguity between the return-column variable import_status
--   and financial_import_batches.import_status
...
begin
  ...
  update public.financial_import_batches as fib
  set import_status = 'ready_for_review'
  where fib.id = v_batch_id
    and fib.import_status = 'uploaded';
