...
-- Status: CORRECTED REVIEW DRAFT ONLY
-- DO NOT EXECUTE IN SUPABASE WITHOUT SEPARATE APPROVAL
--
-- Correction basis:
-- THRIVE_MONTHLY_FINANCIAL_INGESTION_SCHEMA_REVIEW_v0_2.md
--
-- v0.3 correction objectives:
-- - enforce import lifecycle consistency
-- - enforce parsed-row completeness
-- - enforce review-state consistency
-- - strengthen RLS update scope
-- - replace high-risk cascade relationships with restrict behavior
-- - constrain source-row identity format
-- - protect raw source evidence from routine modification
--
...
create table if not exists public.financial_import_batches (
  ...
  constraint financial_import_batches_review_state_consistency
    check (
      (
        import_status in (
          'under_review',
  ...
  constraint financial_import_batches_approval_state_consistency
    check (
      (
  ...
  constraint financial_import_batches_rejection_state_consistency
    check (
      (
...
create table if not exists public.staged_financial_transactions (
  ...
  constraint staged_financial_transactions_row_identity_format
    check (
      source_row_identity ~ '^[0-9a-f]{64}:[1-9][0-9]*$'
    ),
  ...
  constraint staged_financial_transactions_normalized_type_not_blank
    check (
      transaction_type is null
      or length(trim(transaction_type)) > 0
    ),

  constraint staged_financial_transactions_normalized_check_serial_not_blank
    check (
      check_serial is null
      or length(trim(check_serial)) > 0
    ),

  constraint staged_financial_transactions_normalized_merchant_not_blank
    check (
      merchant_name is null
      or length(trim(merchant_name)) > 0
    ),

  constraint staged_financial_transactions_normalized_category_not_blank
    check (
      category_name is null
      or length(trim(category_name)) > 0
    ),

  constraint staged_financial_transactions_normalized_subcategory_not_blank
    check (
      subcategory_name is null
      or length(trim(subcategory_name)) > 0
    ),

  constraint staged_financial_transactions_parsed_completeness
    check (
      parse_status <> 'parsed'
      or (
        ...
        and length(trim(transaction_type)) > 0
        and category_name is not null
        and length(trim(category_name)) > 0
        and subcategory_name is not null
        and length(trim(subcategory_name)) > 0
  ...
  constraint staged_financial_transactions_parse_review_consistency
    check (
      (
        parse_status in (
          'needs_review',
  ...
  constraint staged_financial_transactions_identity_row_consistency
    check (
      case
        when source_row_identity ~ '^[0-9a-f]{64}:[1-9][0-9]*$'
          then split_part(source_row_identity, ':', 2)::bigint
...
create table if not exists public.financial_transaction_reviews (
  ...
  constraint financial_transaction_reviews_completion_requires_reviewer
    check (
      (
  ...
  constraint financial_transaction_reviews_duplicate_state_consistency
    check (
      (
        review_status = 'confirmed_duplicate'
        and duplicate_classification = 'confirmed_duplicate'
      )
      or
      (
        review_status = 'potential_duplicate'
        and duplicate_classification in (
          'content_identical',
          'potential_cross_source_match'
        )
      )
      ...
      (
        ...
        and duplicate_classification = 'not_duplicate'
      )
      or
  ...
  constraint financial_transaction_reviews_trust_match_consistency
    check (
      (
        review_status = 'matched_to_trust_record'
        and trust_match_status = 'matched'
        and trust_record_reference is not null
        and length(trim(trust_record_reference)) > 0
      )
      or
      review_status <> 'matched_to_trust_record'
    ),

  constraint financial_transaction_reviews_matched_reference_required
    check (
      (
        trust_match_status = 'matched'
        and trust_record_reference is not null
        and length(trim(trust_record_reference)) > 0
      )
      or
      (
        trust_match_status <> 'matched'
        and trust_record_reference is null
      )
    ),
...
-- ============================================================
-- 6. Raw-source immutability protection
-- ============================================================

create or replace function public.protect_staged_financial_source_fields()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if new.workspace_id is distinct from old.workspace_id
    ...
    or new.source_row_identity is distinct from old.source_row_identity
    or new.source_content_fingerprint is distinct from old.source_content_fingerprint
    or new.raw_posted_date is distinct from old.raw_posted_date
    or new.raw_transaction_date is distinct from old.raw_transaction_date
    or new.raw_transaction_type is distinct from old.raw_transaction_type
    or new.raw_check_serial is distinct from old.raw_check_serial
    or new.raw_full_description is distinct from old.raw_full_description
    or new.raw_merchant_name is distinct from old.raw_merchant_name
    or new.raw_category_name is distinct from old.raw_category_name
    or new.raw_subcategory_name is distinct from old.raw_subcategory_name
    or new.raw_amount is distinct from old.raw_amount
    or new.raw_daily_posted_balance is distinct from old.raw_daily_posted_balance
...
drop trigger if exists protect_staged_financial_source_fields
  on public.staged_financial_transactions;

create trigger protect_staged_financial_source_fields
before update on public.staged_financial_transactions
for each row
execute function public.protect_staged_financial_source_fields();

-- Normalized fields, lifecycle state, parsing status, and parse-error
-- information may still be updated through authorized review workflows.
--
-- Corrections to source evidence must be handled through a documented
-- replacement or superseding import batch rather than silent mutation.

-- ============================================================
-- 7. Parent-batch identity and approval-state protection
-- ============================================================

create or replace function public.validate_staged_financial_batch_identity()
returns trigger
language plpgsql
set search_path = public
as $$
...
begin
  select
    ...
    import_status
  into
    v_source_file_sha256,
    v_import_status
  from public.financial_import_batches
  ...
  if not found then
    raise exception
      'Matching financial import batch was not found';
  end if;

  if split_part(new.source_row_identity, ':', 1)
      is distinct from v_source_file_sha256
  then
    raise exception
      'Source-row identity does not match the parent batch file fingerprint';
  end if;

  if tg_op = 'INSERT'
    and v_import_status in (
      'approved',
      'rejected'
    )
  then
...
drop trigger if exists validate_staged_financial_batch_identity
  on public.staged_financial_transactions;

create trigger validate_staged_financial_batch_identity
before insert or update on public.staged_financial_transactions
for each row
execute function public.validate_staged_financial_batch_identity();

create or replace function public.protect_approved_financial_batch_records()
returns trigger
language plpgsql
set search_path = public
as $$
declare
  v_import_status text;
begin
  select fib.import_status
  into v_import_status
  from public.financial_import_batches fib
  where fib.id = coalesce(
    new.import_batch_id,
    old.import_batch_id
  );

  if v_import_status = 'approved' then
    raise exception
...
drop trigger if exists protect_approved_staged_financial_transactions
  on public.staged_financial_transactions;

create trigger protect_approved_staged_financial_transactions
before update on public.staged_financial_transactions
for each row
execute function public.protect_approved_financial_batch_records();

create or replace function public.protect_approved_financial_reviews()
returns trigger
language plpgsql
set search_path = public
as $$
...
begin
  select fib.import_status
  into v_import_status
  from public.financial_transaction_reviews ftr
  join public.staged_financial_transactions sft
    on sft.id = ftr.staged_transaction_id
  join public.financial_import_batches fib
    on fib.id = sft.import_batch_id
  where ftr.id = old.id;
...
drop trigger if exists protect_approved_financial_reviews
  on public.financial_transaction_reviews;

create trigger protect_approved_financial_reviews
before update on public.financial_transaction_reviews
for each row
execute function public.protect_approved_financial_reviews();

create or replace function public.protect_financial_import_batch_lifecycle()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if new.workspace_id is distinct from old.workspace_id
    or new.program_id is distinct from old.program_id
    or new.financial_source_id is distinct from old.financial_source_id
    or new.statement_period_start is distinct from old.statement_period_start
    or new.statement_period_end is distinct from old.statement_period_end
    ...
    or new.source_lifecycle is distinct from old.source_lifecycle
    or new.uploaded_by is distinct from old.uploaded_by
    or new.uploaded_at is distinct from old.uploaded_at
    or new.created_at is distinct from old.created_at
  then
    raise exception
      'Financial import batch source identity and lineage are immutable';
  end if;

  if old.import_status = 'approved' then
    raise exception
      'An approved financial import batch is immutable';
  end if;

  if old.import_status = 'rejected' then
    raise exception
      'A rejected financial import batch is immutable';
  end if;
...
drop trigger if exists protect_financial_import_batch_lifecycle
  on public.financial_import_batches;

create trigger protect_financial_import_batch_lifecycle
before update on public.financial_import_batches
for each row
execute function public.protect_financial_import_batch_lifecycle();

create or replace function public.validate_financial_review_batch_state()
returns trigger
language plpgsql
set search_path = public
as $$
...
begin
  ...
  join public.financial_import_batches fib
    on fib.id = sft.import_batch_id
  where sft.id = new.staged_transaction_id
    and sft.workspace_id = new.workspace_id
    and sft.program_id = new.program_id;

  if not found then
    raise exception
      'Matching staged transaction and import batch were not found';
  end if;

  if v_import_status in (
    'approved',
    'rejected'
  )
  then
...
drop trigger if exists validate_financial_review_batch_state
  on public.financial_transaction_reviews;

create trigger validate_financial_review_batch_state
before insert on public.financial_transaction_reviews
for each row
execute function public.validate_financial_review_batch_state();

-- The parent batch fingerprint must match the fingerprint embedded
-- in each staged source-row identity.
--
-- Approved batches are treated as closed audit records.
-- Any later correction must use a documented superseding batch or
-- future amendment workflow rather than rewriting approved evidence.
...
-- ============================================================
-- 10. financial_import_batches RLS
-- ============================================================
...
-- ============================================================
-- 11. staged_financial_transactions RLS
-- ============================================================
...
-- ============================================================
-- 12. financial_transaction_reviews RLS
-- ============================================================
...
-- ============================================================
-- 13. Delete and rollback boundary
-- ============================================================
--
...
-- ============================================================
-- 14. Legal, ethical, and fiduciary boundary
-- ============================================================
--
